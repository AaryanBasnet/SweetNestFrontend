...
export default function Products() {
  ...
  const prepareFormData = (data) => {
    ...
    // 4. New Image Files (THE FIX IS HERE)
    if (data.images && data.images.length > 0) {
      data.images.forEach((img) => {
        // Change 'instanceof File' to 'instanceof Blob' to catch compressed images
        if (img instanceof Blob) {
          // WE MUST PROVIDE A FILENAME for blobs, or Multer might ignore them
          // If the image has a 'name' property (it's a File), use it.
          // If it's a raw Blob, give it a default name like "image.jpg".
          const fileName = img.name || "compressed-image.jpg";
      ...
      render: (value, row) => (
        <div>
          <span className="font-medium">
            Rs. {value || row.weightOptions?.[0]?.price || 0}
          </span>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <select
            ...
            onChange={(e) =>
              setProductsFilter("category", e.target.value || null)
            }
