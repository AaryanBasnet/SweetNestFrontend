...
export default function PriceRangeSlider({
  ...
  const applyFilter = () => {
    let newMin = minValue === '' ? min : Math.max(min, Math.min(Number(minValue), max - 100));
    let newMax = maxValue === '' ? max : Math.min(max, Math.max(Number(maxValue), min + 100));
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1">
          <div className="relative">
            <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-dark/40 text-xs">Rs. </span>
            <input
              type="text"
              value={minValue}
        ...
        <span className="text-dark/30 text-sm">to</span>
        <div className="flex-1">
          <div className="relative">
            <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-dark/40 text-xs">Rs.</span>
            <input
